    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: (args_maxSeq) => {
        ...
        const allEls = Array.from(document.querySelectorAll('div, a, button, li')).filter(el => {
          // Bỏ qua các phần tử thuộc thanh điều hướng, form nhập liệu, popup setting
          if (el.closest('nav, aside, form, [role="navigation"], header, [class*="composer"], [class*="prompt-box"], [class*="sidebar"], [class*="drawer"], [role="dialog"], [role="menu"]')) return false;
          
          const r = el.getBoundingClientRect();
          if (r.left < 260) return false; // Cột bên trái
          ...
          // ĐIỀU KIỆN SỐNG CÒN: Chỉ quét Media
          const hasMediaId = el.hasAttribute('data-media-id') || el.hasAttribute('data-workflow-id') || el.hasAttribute('data-id') || el.querySelector('[data-media-id], [data-workflow-id]');
          const hasMediaTag = el.querySelector('video, img');
          const hasSpinner = el.querySelector("[role='progressbar'], svg.animate-spin, .animate-spin, svg[class*='spin']");
          const t = (el.innerText || el.textContent || '').toLowerCase();
          const isGenerating = /\b\d+\s*%/i.test(t) || t.includes('đang tạo') || t.includes('generating');
          const isFailed = t.includes('không thành công') || t.includes('vi phạm') || t.includes('failed');
          
          // Tuyệt đối loại bỏ Empty State
          if (t.includes('bắt đầu tạo') || t.includes('thả nội dung')) return false;
          
          // Nếu không có mảy may dấu hiệu nào của thẻ media thì loại
          const seqMatchStr = t + " " + (el.getAttribute('title') || '') + " " + (el.getAttribute('aria-label') || '');
          const hasSeq = /(?:^|\s)(\d{1,4})[\.\-_:\s]/.test(seqMatchStr);
          
          if (!hasMediaId && !hasMediaTag && !hasSpinner && !isGenerating && !isFailed && !hasSeq) return false;
