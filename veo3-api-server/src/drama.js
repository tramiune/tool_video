...
async function generateDramaScript({ topic, channelType = 'drama' }) {
  ...
  if (channelType === 'sumo') {
    ...
    userPrompts = [
      ...
      '- Trong baseImagePrompt, imagePrompt và videoPrompt của TẤT CẢ các cảnh, PHẢI mô tả cực kỳ rõ ràng vị trí đứng cố định sát hai bên rìa của từng nhân vật bằng tiếng Anh (ví dụ: "Bin is standing completely static on the far left side, Sumo is standing completely static on the far right side. Sumo is a friendly deer character wearing a red cape and a red bowtie on his neck, standing upright on two legs. Both characters remain fixed in their spots, facing forward and keeping direct eye contact with the camera/viewer at all times. They must never turn their face away, look aside, or turn their head. They talk with subtle facial expressions and natural lip movements, without walking or shifting positions. Once the dialogue speech ends, they must freeze completely in their final pose, staying 100% still and looking straight ahead at the camera for the rest of the video without any body movement, actions, gestures, or turning away. Do not write or include any text, labels, subtitles, names as text, or words inside the visual outputs. The output must be completely clean and free of any text overlay"). Giữ nguyên vị trí cực hạn cố định này nhất quán xuyên suốt các cảnh.',
