...
async function generateDramaScript({ topic, channelType = 'drama' }) {
  ...
  if (channelType === 'sumo') {
    ...
    userPrompts = [
      ...
      '- Cảnh 1 (Hook): Bin đưa ra một tuyên bố ngộ nhìn hoặc lười ăn rau/món ăn đa dạng trong bữa ăn/buổi dã ngoại. Bạn học hoặc mẹ ngạc nhiên. Chú hươu Sumo xuất hiện bất ngờ.',
      '- Cảnh 2 (Thắc mắc/Tranh luận): Bin hỏi Sumo tại sao phải ăn nhiều thứ. Sumo dùng kiến thức khoa học vui nhộn giải thích vai trò của dinh dưỡng (carbohydrate, protein, chất béo, vitamin, khoáng chất và chất xơ).',
      '- Cảnh 3 (Hậu quả kịch tính nhẹ): Sumo giải thích điều gì xảy ra nếu cơ thể thiếu chất xơ, vitamin từ rau củ quả (bé dễ mệt mỏi, khó tiêu, hệ tiêu hóa quá tải). Bin hơi lo lắng.',
      '- Cảnh 4 (Thử thách đĩa ăn cầu vồng): Sumo đưa ra một thử thách thú vị chọn đồ ăn đa sắc màu (rau xanh, cơm, trứng, trái cây) để tạo đĩa ăn cân đối. Bin hào hứng thực hiện.',
      '- Cảnh 5 (Quảng cáo sản phẩm): Bin băn khoăn nếu có hôm bận rộn/ăn chưa đủ chất thì sao. Sumo giới thiệu sản phẩm GẠC HƯƠU NON SUMO (chứa nhung hươu cô đặc, Vitamin C, Kẽm, Fructooligosaccharides và hỗn hợp 5 loại trái cây nhiệt đới: cam, xoài, ổi, chuối, chanh dây) giúp bổ sung dinh dưỡng dễ dàng và thơm ngon.',
      '- Cảnh 6 (Kết thúc & Tương tác): Bin thay đổi suy nghĩ, ăn uống đa dạng hơn. Bin và Sumo hướng về camera đưa ra câu hỏi tương tác thú vị đố khán giả xem video để kích thích bình luận.',
