...
function App() {
  ...
  useEffect(() => {
    ...
    setDoc(userDocRef, { email: user.email || 'guest@meo3.cloud', createdAt: Date.now() }, { merge: true }).catch(err => {
  ...
  const generateAudio = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/audio/generate`, {
        ...
        body: JSON.stringify({
          userId: user.uid,
          userEmail: user.email || 'guest@meo3.cloud',
          text: audioText,
          ...
          <div style={{ position: 'relative' }} ref={userDropdownRef}>
            ...
            >
              {user.photoURL ? (
                ...
                <span>{(user.email || 'G')[0].toUpperCase()}</span>
              )}
            </div>
